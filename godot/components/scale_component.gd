...
# Export the base scale
@export var base_scale: Vector2 = Vector2.ONE

# Export the scale amount multiplier
...
func tween_scale() -> void:
	var scale_amount = base_scale * multiplier
	...
	tween.tween_property(sprite, "scale", base_scale, scale_duration * 0.9).from(scale_amount)
